...
log(`\n${c.b}── O que mudou na v1.2.0 ──────────────────────────────────────${c.x}`)
log(`${c.g}px-handoff${c.x} — entrega redesenhada para projetos reais:`)
log(`${c.d}  · Pacote entregue ao dev: HTML do protótipo + UI Kit + histórias de negócio${c.x}`)
log(`${c.d}  · Rastreabilidade dupla: texto descritivo + data-story="<ID>" no elemento HTML${c.x}`)
log(`${c.d}  · GATE de saída obrigatório — nada sai sem passar pela barreira de qualidade${c.x}`)
log(`${c.d}  · Push direto no repo do dev (com aceite explícito do PX antes de executar)${c.x}`)
log(`${c.d}  · Removido: branches de código, config, artefatos internos (PX-PROGRESS)${c.x}`)
log(`${c.d}  · DoD virou checklist interna da skill — não vai mais no pacote do dev${c.x}`)
log(`\n${c.g}px-intake · px-story${c.x} — ajuste de nomenclatura:`)
log(`${c.d}  · agile-epic → px-epic · agile-proto → px-proto${c.x}`)
log(`${c.b}────────────────────────────────────────────────────────────────${c.x}\n`)
