...
log(`\n${c.b}── O que mudou na v1.4.2 ──────────────────────────────────────${c.x}`)
log(`${c.g}px-proto${c.x} — gate de ambiguidade e inventário de componentes (aprendizado de campo):`)
log(`${c.d}  · Passo 1 (novo): inventário público de todos os widgets → componente shadcn${c.x}`)
log(`${c.d}  · Passo 2 (novo): gate explícito para ambiguidades de variação antes de codar${c.x}`)
log(`${c.d}  · Seletor de período, ícone decorativo vs funcional, pills clicáveis ou display?${c.x}`)
log(`${c.d}  · Elemento clicável sem ação definida na spec → sinalizar, nunca inventar${c.x}`)
log(`${c.d}  · Tabela nativa / Badge manual / Tooltip ausente: proibidos quando shadcn existe${c.x}`)
log(`${c.d}  · Regra: ambiguidade resolvida em silêncio = retrabalho garantido${c.x}`)
log(`${c.b}────────────────────────────────────────────────────────────────${c.x}\n`)
