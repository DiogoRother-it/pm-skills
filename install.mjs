...
log(`\n${c.b}── O que mudou na v1.3.0 ──────────────────────────────────────${c.x}`)
log(`${c.g}px-handoff${c.x} — correções baseadas em execução real (Sprint 29):`)
log(`${c.d}  · HTML sempre unificado — nunca separado por funcionalidade${c.x}`)
log(`${c.d}  · Push via branch órfã — entrega limpa sem herdar histórico do boilerplate${c.x}`)
log(`${c.d}  · data-story em elementos gerados por JS: modificar a função geradora${c.x}`)
log(`${c.d}  · Nomes de histórias sem prefixos numéricos — verificado no GATE${c.x}`)
log(`${c.d}  · pasta de entrega renomeada: handoff-ux/ (raiz do repo do dev)${c.x}`)
log(`${c.d}  · Removidas todas as referências a planning/ da skill${c.x}`)
log(`${c.d}  · Delta automático: skill identifica histórias prontas não entregues${c.x}`)
log(`${c.b}────────────────────────────────────────────────────────────────${c.x}\n`)
