...
log(`\n${c.b}── O que mudou na v1.4.1 ──────────────────────────────────────${c.x}`)
log(`${c.g}px-proto${c.x} — roda no boilerplate (Vite + localhost), não em HTML standalone:`)
log(`${c.d}  · Usa componentes reais do shadcn (Button, Table, Dialog...)${c.x}`)
log(`${c.d}  · Tokens reais do src/index.css — sem extração manual${c.x}`)
log(`${c.d}  · HMR ativo — ajuste aparece sem recarregar${c.x}`)
log(`${c.d}  · Vive em src/proto/<slug>.tsx com rota /proto/<slug>${c.x}`)
log(`${c.d}  · Switcher de estados integrado no componente${c.x}`)
log(`${c.d}  · Arquivo permanece como referência visual pro dev; removido após implementação${c.x}`)
log(`${c.g}px-preview${c.x} — reposicionado: decide formato de compartilhamento externo:`)
log(`${c.d}  · Quem constrói usa localhost (Vite) — o px-preview é sobre entrega externa${c.x}`)
log(`${c.d}  · Avalia projeto e recomenda: HTML standalone, Netlify/Vercel, ou Artifact${c.x}`)
log(`${c.d}  · Suporte a deploy Netlify adicionado (URL pública permanente)${c.x}`)
log(`${c.g}Fluxo completo:${c.x}`)
log(`${c.d}  px-request → px-proto [obrigatório, localhost] → px-story → [px-preview opcional] → px-handoff${c.x}`)
log(`${c.b}────────────────────────────────────────────────────────────────${c.x}\n`)
